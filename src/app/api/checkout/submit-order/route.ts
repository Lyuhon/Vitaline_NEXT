...
// // src/app/api/checkout/submit-order/route.ts
// import { NextRequest, NextResponse } from 'next/server';
// import { createOrder } from '../../../../lib/woocommerce'; // Путь может отличаться (см. структуру вашего проекта)

// // Функция декодирования Base64 ID из GraphQL (например, "cG9zdDoxMTEyOA==")
// function decodeProductId(encodedId: string): number {
//     try {
//         // Декодируем строку из Base64: "post:11128"
//         const decodedString = Buffer.from(encodedId, 'base64').toString('utf-8');
//         // Разделяем строку по двоеточию ["post", "11128"]
...
// export async function POST(request: NextRequest) {
//     try {
//         // 1. Получаем данные заказа от клиента (CheckoutContext)
//         const orderData = await request.json();
//         console.log('Получены данные заказа:', JSON.stringify(orderData, null, 2));

//         // 2. Проверяем наличие необходимых данных (опционально)
...
//         // 3. Формируем данные для WooCommerce REST API
//         // Декодируем productId из Base64 → числовой ID
...
//             status: 'processing', // <-- Добавьте строку
//             billing: {
...
//         // 5. Отправка данных в Telegram (опционально)
//         const telegramResponse = await fetch('https://nuxt.vitaline.uz/wp-content/api/web_tg_test.php', {
//             method: 'POST',
//             headers: {
//                 'Content-Type': 'application/json',
//             },
